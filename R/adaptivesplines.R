...
#' @param nknots Number of knots (the knots will be put uniformely in the period)
...
#' @param precision Precision computed by the norm2 of the difference between two successive sets of coefficients of the splines. The iterations are stopped if that norm is lower than the precision
#' @param threshold Threshold used to select a knot, based on the z-values (near 0 or 1 if the knot is discarded or selected); for instance .99.
#'
#' @return
#' @export
#'
...
.periodic_adaptive_spline<-function(x, y, period, nknots, knots=NULL, order=4, lambda=0.1, maxiter=20, precision=0.001, threshold=0.98){
  ...
  jrslt<-.jcall('jdplus/advancedsa/base/r/AdaptivePeriodicSplines', 'Ljdplus/advancedsa/base/r/AdaptivePeriodicSplines$AdaptivePSpline;', 'aspline',
...
.periodic_adaptive_splines<-function(x, y, period, nknots, knots=NULL, order=4, maxiter=20, precision=0.001, threshold=0.98,
  ...
  jrslt<-.jcall('jdplus/advancedsa/base/r/AdaptivePeriodicSplines', 'Ljdplus/advancedsa/base/r/AdaptivePeriodicSplines$AdaptivePSplines;', 'asplines',
