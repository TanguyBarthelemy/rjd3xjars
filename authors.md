# Authors and Citation

## Authors

- **Jean Palate**. Author.

- **Tanguy Barthelemy**. Author, maintainer.

## Citation

Source:
[`DESCRIPTION`](https://github.com/rjdverse/rjd3xjars/blob/develop/DESCRIPTION)

Palate J, Barthelemy T (2026). *rjd3xjars: R Interface to 'JDemetra+
3.x' Time Series Analysis Software*. R package version 0.1.0,
<https://github.com/rjdverse/rjd3xjars>.

    @Manual{,
      title = {rjd3xjars: R Interface to 'JDemetra+ 3.x' Time Series Analysis Software},
      author = {Jean Palate and Tanguy Barthelemy},
      year = {2026},
      note = {R package version 0.1.0},
      url = {https://github.com/rjdverse/rjd3xjars},
    }
